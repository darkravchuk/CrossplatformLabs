...
public class ApiClient
{
    ...
    public async Task<List<HousingBenefitViewModel>> GetHousingBenefitsAsync()
    {
        var response = await _httpClient.GetAsync("/api/housing-benefits");
        response.EnsureSuccessStatusCode();
        return await response.Content.ReadFromJsonAsync<List<HousingBenefitViewModel>>();
    }

    public async Task<HousingBenefitViewModel> GetHousingBenefitByIdAsync(int id)
    {
        var response = await _httpClient.GetAsync($"/api/housing-benefits/{id}");
        response.EnsureSuccessStatusCode();
        return await response.Content.ReadFromJsonAsync<HousingBenefitViewModel>();
    }

    public async Task<HousingBenefitViewModel> CreateHousingBenefitAsync(HousingBenefitViewModel newHousingBenefit)
    {
        var response = await _httpClient.PostAsJsonAsync("/api/housing-benefits", newHousingBenefit);
        response.EnsureSuccessStatusCode();
        return await response.Content.ReadFromJsonAsync<HousingBenefitViewModel>();
    }

    public async Task UpdateHousingBenefitAsync(int id, HousingBenefitViewModel updatedHousingBenefit)
    {
        var response = await _httpClient.PutAsJsonAsync($"/api/housing-benefits/{id}", updatedHousingBenefit);
        response.EnsureSuccessStatusCode();
    }

    public async Task DeleteHousingBenefitAsync(int id)
    {
        var response = await _httpClient.DeleteAsync($"/api/housing-benefits/{id}");
        response.EnsureSuccessStatusCode();
    }
}